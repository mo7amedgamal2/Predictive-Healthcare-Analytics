...
# Dataset path - try data directory first (for deployment), then fallback to Milestone -2
DATASET_PATH = Path("data/oral_cancer_prediction_dataset.csv")
...
def load_dashboard_data():
    ...
    possible_paths = [
        Path("data/oral_cancer_prediction_dataset.csv"),  # Deployment path
        Path("archive/Milestone -2/oral_cancer_prediction_dataset (Before Milestone-2).csv"),  # Archive path
        Path("archive/Milestone -2/oral_cancer_prediction_dataset(Pre-model)not_scale.csv"),  # Alternative archive path
    ]
