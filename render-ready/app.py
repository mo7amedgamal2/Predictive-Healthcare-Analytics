...
def predict_integer(model, data):
    ...
    if isinstance(predictions, np.ndarray):
        ...
        rounded = max(0, int(np.round(predictions)))
    return rounded

# Configure logging - use stdout only for production (Fly.io)
logging.basicConfig(
    level=logging.INFO,  # Use INFO for production
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout)  # Only stdout for Fly.io
    ]
)
logger = logging.getLogger(__name__)

# Get base directory (parent of this file, since app.py is in root)
BASE_DIR = Path(__file__).resolve().parent

app = Flask(__name__)
app.config['SECRET_KEY'] = 'oral-cancer-prediction-secret-key'

# Initialize model loader with relative path
...
treatment_encoder_path = models_dir / "treatment_type_encoder.pkl"
...
for model_name in ['cancer_label', 'cost', 'los', 'recovery', 'survival_rate']:
    feature_order_path = models_dir / f"{model_name}_feature_order.pkl"
...
# Load models in background thread to prevent startup timeout
_models_loading = False
_models_loaded = False

def load_models_background():
    """Load models in background thread"""
    global _models_loading, _models_loaded, loaded_models, model_status
...
def ensure_models_loaded():
    ...
    global loaded_models, model_status, _models_loaded
    # Wait for models to load if still loading (with timeout)
    if not _models_loaded and not loaded_models:
        import time
        timeout = 30  # 30 second timeout
        start = time.time()
        while not _models_loaded and (time.time() - start) < timeout:
            time.sleep(0.1)
    # Always refresh status to ensure it's up-to-date
    if loaded_models:
        model_status = {k: v.get('status', 'UNKNOWN') for k, v in loaded_models.items()}
...
def predict_cost():
    ...
    try:
        ...
        # Ensure prediction is non-negative (cost cannot be negative)
...
def predict_los():
    ...
    try:
        ...
        # Ensure prediction is non-negative (LOS cannot be negative)
...
def predict_recovery():
    ...
    try:
        ...
        # Ensure prediction is non-negative (Recovery time cannot be negative)
        prediction = max(0, int(prediction))
...
def get_model_status():
    ...
    global model_status
    # Refresh status from loaded_models
    if loaded_models:
        model_status = {k: v.get('status', 'UNKNOWN') for k, v in loaded_models.items()}
    ...
    # Also include models that might not be loaded yet
    expected_models = ['cancer_label', 'cost', 'survival_rate', 'los', 'recovery']
